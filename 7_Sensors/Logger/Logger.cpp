...
#if USE_MIDI_UART
void Logger::sendNoteOn(uint8_t channel, uint8_t note, uint8_t velocity)
{
    UARTCharPut(UART0_BASE, 0x90 | (channel & 0x0F));
    UARTCharPut(UART0_BASE, note);
    UARTCharPut(UART0_BASE, velocity & 0x7F);
    AudioOutput::getInstance().getAudioMixer()->setMIDINote(note);
}

void Logger::sendNoteOff(uint8_t channel, uint8_t note, uint8_t velocity)
{
    UARTCharPut(UART0_BASE, 0x80 | (channel & 0x0F));
    UARTCharPut(UART0_BASE, note);
    UARTCharPut(UART0_BASE, velocity & 0x7F);
}

void Logger::controlChange(uint8_t channel, uint8_t controller, uint8_t value)
{
    UARTCharPut(UART0_BASE, 0xB0 | (channel & 0x0F));
    UARTCharPut(UART0_BASE, controller);
    UARTCharPut(UART0_BASE, value & 0x7F);
}

void Logger::setInstrument(uint8_t channel, uint8_t type)
{
    UARTCharPut(UART0_BASE, 0xC0 | (channel & 0x0F));
